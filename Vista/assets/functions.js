$(document).ready(function () {
  ...
  $("#btn_add_new_file").click(function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      contentType: false,
      beforeSend: function () {
        $("#loader_add_new_legal").show();
        $(".adding_new_legal_content").hide();
      },
      success: function (response) {
        console.log(response);
        // if (response) {
      ...
      complete: function () {
        setTimeout(() => {
          $(".adding_new_legal_content").show();
          $("#loader_add_new_legal").hide();
          $("#modal_upld_new_File").modal("hide");
        }, 2000);
      },
      error: function (xhr, status, error) {
        console.error(error);
